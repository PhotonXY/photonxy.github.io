...
function renderToday(){
  ...
  DB.acts.forEach(a=>{
    ...
    rows.push(`
        <tr class="${rowCls} click-to-comment" data-activity="${a.id}" data-due="${due}">
            <td>${r.name}</td>
            <td>
                ${a.title}
  ...
  // Zeilenklick: Kommentar ein-/ausklappen
  if (!tbody._rowBound) tbody.addEventListener('click', (e) => {
    const tr = e.target.closest('tr');
    if (!tr || tr.classList.contains('comment-row')) return;
    if (e.target.closest('button')) return; // Buttons ignorieren
    const actId = tr.getAttribute('data-activity');
    if (!actId) return;
    const row = document.getElementById(`comment-row-${actId}`);
    if (!row) return;
    const isVisible = row.style.display === 'table-row';
    row.style.display = isVisible ? 'none' : 'table-row';
    if (!isVisible) {
      const ta = document.getElementById(`comment-${actId}`);
      if (ta) ta.focus();
    }
...
function renderOverdue() {
    ...
    DB.acts.forEach(a => {
        for (let checkDay of pastDays) {
            ...
            const canQuit = (state.role === 'pruefer' || state.role === 'admin');

            const statusHtml = '<span class="status-open">Versäumt</span>';
            const schedHtml = describeSched(a.sched);

            rows.push(`
                <tr class="overdue-row click-to-comment${hasComment ? ' has-comment' : ''}" data-activity="${a.id}" data-due="${checkDay}">
                    ...
                    <td>${statusHtml}</td>
                    <td>
                        ${canQuit ? `<button class="small-button quit-btn" onclick="event.stopPropagation(); quitOverdue('${a.id}', '${checkDay}')" ${hasComment ? '' : 'disabled'}>Quittieren</button>` : ''}
                    </td>
                </tr>
    ...
    if (!tbody._rowBound) tbody.addEventListener('click', (e) => {
      ...
      const actId = tr.getAttribute('data-activity');
      if (!actId) return;
      const row = document.getElementById(`overdue-comment-row-${actId}`);
      if (!row) return;
      const isVisible = row.style.display === 'table-row';
      row.style.display = isVisible ? 'none' : 'table-row';
      if (!isVisible) {
        const ta = document.getElementById(`overdue-comment-${actId}`);
        if (ta) ta.focus();
      }
    }); tbody._rowBound = true;
    // also update mobile cards
      ...
      rows.push(`
        <div class="task-card">
          ...
          <div class="task-actions">
            ...
            ${canQuit?`<button class="small-button quit-btn" onclick="openSheet('overdue','${a.id}','${checkDay}', true)" ${commentCount?'' :'disabled'}>Quittieren</button>`:''}
          </div>
        </div>`);
      break;
    }
...
function renderLogs(){
  ...
  tbody.innerHTML = slice.map(l=>{
    ...
    const hasComments = DB.comments.some(c=> c.activityId===l.activityId && c.dueDate===l.dueDate);
    const headRow = `<tr ${hasComments?`class=\"click-to-comment\" data-activity=\"${l.activityId}\" data-due=\"${l.dueDate}\"`:''}>
        <td>${when}</td>
        <td>${u?u.name:'—'}</td>
        <td>${r?r.name:'—'}</td>
        <td>${a?a.title:'—'}</td>
        <td>${l.dueDate}</td>
        <td>${hasComments?`<span class=\"comment-indicator clickable\" onclick=\"toggleLogComments('${l.activityId}','${l.dueDate}')\"></span>`:''}</td>
      </tr>`;
    const commentRow = hasComments ? `<tr id=\"${cid}\" class=\"comment-row\" style=\"display:none; background: var(--bg);\"><td colspan=\"6\">${renderCommentHistoryChrono(l.activityId, l.dueDate)}</td></tr>` : '';
    return headRow + commentRow;
  }).join('') || `<tr><td colspan=\"6\" class=\"muted\">Keine Protokolle.</td></tr>`;
  // Row click toggle only when comments exist
  if (!tbody._rowBound){
    tbody.addEventListener('click', (e)=>{
      const tr = e.target.closest('tr'); if (!tr || tr.classList.contains('comment-row')) return;
      const act = tr.getAttribute('data-activity'); const due = tr.getAttribute('data-due');
      if (!act || !due) return; // only rows with comments have data-attrs
      const row = document.getElementById(`log-comments-${act}-${due}`); if (!row) return;
      const vis = row.style.display === 'table-row'; row.style.display = vis ? 'none' : 'table-row';
    });
    tbody._rowBound = true;
  }
  renderLogsCards();
...
function renderAcks(){
  ...
  tbody.innerHTML = items.map(a=>{
    ...
    const headRow = `<tr ${hasComments?`class=\"click-to-comment\" data-activity=\"${a.activityId}\" data-due=\"${a.dueDate}\"`:''}>
        <td>${when}</td>
        <td>${pruefer}</td>
        ...
        <td>${typ}</td>
        <td>${details}</td>
        <td>${hasComments?`<span class=\"comment-indicator clickable\" onclick=\"toggleAckComments('${a.activityId}','${a.dueDate}')\"></span>`:''}</td>
      </tr>`;
    const commentRow = hasComments ? `<tr id=\"${cid}\" class=\"comment-row\" style=\"display:none; background: var(--bg);\">\n        <td colspan=\"7\">\n          <div class=\"ack-info\">${info}</div>\n          ${comments}\n        </td>\n      </tr>` : '';
    return headRow + commentRow;
  ...
  if (!tbody._rowBound){
    tbody.addEventListener('click', (e)=>{
      ...
      const act = tr.getAttribute('data-activity'); const due = tr.getAttribute('data-due');
      if (!act || !due) return;
      const row = document.getElementById(`ack-comments-${act}-${due}`); if (!row) return;
      const vis = row.style.display === 'table-row'; row.style.display = vis ? 'none' : 'table-row';
    });
    tbody._rowBound = true;
  }
...
function openSheet(context, activityId, dueDate, wantQuit){
  ...
  if (sec){
    const canQuit = (context==='overdue') && (state.role==='pruefer'||state.role==='admin');
    sec.style.display = canQuit ? '' : 'none';
    sec.disabled = !canQuit;
  }
...
function quitOverdue(activityId, dueDate) {
    if (state.role !== 'pruefer' && state.role !== 'admin') {
        alert('Nur Prüfer und Administratoren dürfen quittieren.');
        return;
    }
